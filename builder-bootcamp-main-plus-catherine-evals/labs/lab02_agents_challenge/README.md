...
> **Note:** Getting stuck?
>
> You’re encouraged to work through the tasks on your own first. If you need a hint or want to compare approaches:
> - You can peek at the local reference files in this folder (e.g., `*_solution.py`).
> - Or run the solution variant using `--solution` with the runner to see a working trace.
> - For a more guided walkthrough, see `labs/lab02_agents_guided/`.
...
3. Compose agents using agent‑as‑tool so a central support agent can delegate FAQ questions to a KnowledgeAssistant.
...
6. Generate a JSONL results file over a sample suite and evaluate results with the Evals API.
...
- Use the checkpoints/expected outputs to verify you’re on track before moving on.
...
**The challenge**
- Deliver an end‑to‑end prototype in **<3 hours** with minimal scaffolding.
...
Walk through these files so you know where each step reads and writes.
...
**Where to work:** `labs/lab02_agents_challenge/tools.py`
...
```bash
python - << 'PY'
from labs.lab02_agents_challenge import tools
...
from labs.lab02_agents_challenge import tools  # your implementation
...
import asyncio
from agents import Agent, Runner
from labs.lab02_agents_challenge.tools import raise_complaint
...
import asyncio
from agents import Agent, Runner
from labs.lab02_agents_challenge.tools import check_payment_methods
...
import asyncio
from agents import Agent, Runner
from labs.lab02_agents_challenge.tools import FAQ_retrieval
...
```bash
grep -q 'Guidelines:' labs/lab02_agents_challenge/agent.py && \
grep -q 'Call exactly one tool at a time' labs/lab02_agents_challenge/agent.py && \
grep -q 'Never list the tools you have access to' labs/lab02_agents_challenge/agent.py && \
...
```powershell
if (
    (Select-String -Path 'labs/lab02_agents_challenge/agent.py' -Pattern 'Guidelines:') -and
    (Select-String -Path 'labs/lab02_agents_challenge/agent.py' -Pattern 'Call exactly one tool at a time') -and
    (Select-String -Path 'labs/lab02_agents_challenge/agent.py' -Pattern 'Never list the tools you have access to')
) {
    Write-Host "PASS"
} else {
    Write-Host "FAIL"
...
python - << 'PY'
import asyncio
from labs.lab02_agents_challenge.agent import build_baseline_agent, run_once
...
```bash
python -m labs.lab02_agents_challenge.agent 
```
...
Now that your baseline agent is working well, let’s shift gears and focus on multi-agent composition. From here on, you’ll be working in `labs/lab02_agents_challenge/router.py`. This file contains all the scaffolding for the next set of patterns: agent‑as‑tool handoff, triage routing, and input guardrails. 
...
```bash
python -m labs.lab02_agents_challenge.run --variant AGENT_AS_TOOL
```
...
> ```bash
> python -m labs.lab02_agents_challenge.run --variant AGENT_AS_TOOL --solution
> ```
...
```bash
python -m labs.lab02_agents_challenge.run --variant TRIAGE
```
...
> ```bash
> python -m labs.lab02_agents_challenge.run --variant TRIAGE --solution
> ```
...
```bash
python -m labs.lab02_agents_challenge.run --variant GUARDRAIL
```
...
```powershell
(Get-Content 'labs/data/agent_model_answer.jsonl' | Select-Object -Index 15) |
  ConvertFrom-Json |
...
```bash
python -m labs.lab02_agents_challenge.eval_agents
```
...
The evaluator loads your JSONL and applies the rubric defined in `labs/lab02_agents_challenge/testing_criteria.py`. By default, two graders are enabled: Q/A relevance and Tool alignment. Answer alignment is optional and can be re‑enabled by uncommenting it in the rubric. After scoring, the evaluator prints a pass/total summary and provides a link to the dashboard for item‑level review.
...
If you're looking for more practice, try one or both of the following:
- Explore the open-ended extension questions in `EXTENSIONS_README.md`. You can ask the facilitators for advice if you get stuck.
- **Try Agent Builder:** Recreate this agentic workflow using [OpenAI Agent Builder](https://platform.openai.com/agent-builder). Experiment with configuring tools, routing, and handoffs visually (or via config) and compare the experience to your code-based solution.
